...
import com.demonorium.database.repository.AccessRepository;
...
public class StorageController {
    ...
    public String generateAccessToken(Note note) {
        ...
        String user_hash = md5(user.getUsername()+user.getEmail() + date.getTime());
        String note_hash = md5(note.getName() + note.getContent() + Long.toString(date.getTime(), 7));

        return rndInsert(user_hash + note_hash, first, second, Long.toHexString(date.getTime())) + first;
    }
